...
func _ready():
	...
	pass
	$MaxPopulationLabel.text = "Max neutron population: %d" % $ViewportContainer/Viewport/Simulation/ReactorCore/NeutronField.get_max_population()
"""
...
func _process(delta):
	...
	var flux := $ViewportContainer/Viewport/Simulation/ReactorCore/NeutronField.get_neutron_flux() as int
	$NeutronFlux/Readout.text = "Nuetron Flux: %d" % flux
	$NeutronFlux/Graph.write_data((flux * 2) + 1) # +1 so it's visible when zero
	
	var neutrons: int = $ViewportContainer/Viewport/Simulation/ReactorCore/NeutronField.num_neutrons()
	var scaleTrip := 500
	var scale := 1
